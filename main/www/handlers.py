...
from main.www.apis import Page, APIValueError, APIError
...
def demo(self):
    ...
    user = users[0]
    print(user.id)
    cook = user2cookie(user, 86400)
    r = web.Response()
    r.set_cookie(COOKIE_NAME, cook, max_age=86400, httponly=True)
    r.content_type = 'application/json'
    r.body = json.dumps(users, ensure_ascii=False).encode('utf-8')
    return r
    # return web.Response(body=b'<h1>Demo</h1>')
...
def api_get_users(self, *, page='1'):
    ...
    r = web.Response()
    r.content_type = 'application/json'
    r.body = json.dumps(t, ensure_ascii=False).encode('utf-8')
    return r
